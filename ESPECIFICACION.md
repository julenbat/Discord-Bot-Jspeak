...
| Opus | `opusscript@0.0.8` **exacto** (peer de prism-media `^0.0.8`; 0.1.1 rompe `npm ci` solo en Docker). **Un encoder de larga vida por guild** con `.delete()` al salir — uno por locución agota el heap WASM en la locución ~55 (medido). Gauge de encoders vivos. `@discordjs/opus` descartado en Node 24 (sin prebuilds ABI 137, SIGSEGV #206). |
...
| Player | Se hereda del hermano el prebuffer y la disciplina de cancelación; **NO el reloj de 20 ms** (`@discordjs/voice` ya re-pacea; dos relojes con deriva independiente se oyen). `NoSubscriberBehavior.Play`, `maxMissedFrames ~25`. |
